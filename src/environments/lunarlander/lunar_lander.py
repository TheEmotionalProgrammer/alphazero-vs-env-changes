...
def clone_body(current, other):
    """
    Clone the properties of one Box2D body into another.
    """

    current.awake = other.awake
    current.angularVelocity = other.angularVelocity
    current.linearDamping = other.linearDamping
    current.angularDamping = other.angularDamping
    current.gravityScale = other.gravityScale
...
class CustomLunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        # Initialize wind and torque indices (even if wind is disabled)
        self.wind_idx = 0
        self.torque_idx = 0
    ...
    def reset(
        ...
        *,
        ...
        self.drawlist = [self.lander] + self.legs + self.floating_terrain
    ...
    def create_copy(self):
        ...
        if self.particles is not None:
            for particle in self.particles:
                new_particle = new_env._create_particle(
                    particle.mass, particle.position[0], particle.position[1], 0
                )
...
if __name__ == "__main__":
    ...
    gym.register(
        ...
        entry_point=__name__ + ":CustomLunarLander",
