...
FIXED_INIT_STATE = np.array([-18, 18, -PI/2, 4, 4.8, 1.8, 0])
RAND_INIT_STATE = np.array([0, 10, -PI, 4, 4.8, 1.8, 0])
    ...
    def __init__(
        ...
        if observation_type == "vector":
            self.observation_space = spaces.Box(
                -1.0, 1.0, (self.stationary.shape[0] + 2, 13), dtype=np.float32
            )
        ...
        if action_type == "discrete":
            self.action_space = spaces.Discrete(N_SAMPLES_LON_ACTION*N_SAMPLES_LAT_ACTION)
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        if action is not None:
            ...
            if self.action_type == "discrete":
                lon_idx = action // N_SAMPLES_LAT_ACTION
                lat_idx = action % N_SAMPLES_LAT_ACTION
                action = np.array([LON_ACTIONS[lon_idx], LAT_ACTIONS[lat_idx]])
            ...
            kinematic_act(action, next_state, DT)
