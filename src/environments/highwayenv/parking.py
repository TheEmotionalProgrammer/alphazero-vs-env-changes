...
class ParkingEnv(AbstractEnv, GoalEnv):
    ...
    def compute_reward(
        ...
        # p,
        # )
        # # Invert the reward to make it positive and higher as the agent gets closer
        # return 1 / (1 + distance)
...
class ParkingEnvParkedVehicles(ParkingEnv):
    def __init__(self, render_mode: str | None = None) -> None:
        super().__init__({"vehicles_count": 10}, render_mode)
...
if __name__ == "__main__":
    ...
    gym.register(
        id="ParkingEnvParkedVehicles",
        entry_point="parking:ParkingEnvParkedVehicles",
    )

    env = gym.make("ParkingEnv", config={
    ...
    while not terminated:
        obs, reward, terminated, truncated, _ = env.step(env.action_space.sample())  # with manual control, these actions are ignored
        total_reward += reward
