...
if __name__ == "__main__":   
    ...
    plt.fill_between(envs, np.array(mean_false) - np.array(std_false), np.array(mean_false) + np.array(std_false), alpha=0.1)
    plt.plot(envs, mean_true)#, label="$y^{max}$")
    plt.fill_between(envs, np.array(mean_true) - np.array(std_true), np.array(mean_true) + np.array(std_true), alpha=0.1)
    plt.title("Accuracy Error (↓)")
    #plt.legend()
    plt.xticks(np.arange(len(custom_labels)), custom_labels, fontsize=12)
